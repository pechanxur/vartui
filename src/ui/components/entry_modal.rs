...
pub fn render_add_entry_modal(frame: &mut Frame, app: &mut App) {
...

    {
        ...
        let render_field = |f: &mut Frame, title: &str, value: &str, field: FormField, area: Rect| {
        ...
        let b_style = if is_billable_focused { style_focused } else { style_normal };
        let b_sym = if form.is_billable { "[x]" } else { "[ ]" };
        let b_p = Paragraph::new(format!("{} Es facturable (espacio)", b_sym))
            .block(Block::default().borders(Borders::ALL).title("Opciones").border_style(b_style))
            .style(b_style);
    ...
    if let Some((area_ref, indices)) = dropdown_info {
        ...
        frame.render_stateful_widget(list, dropdown_area, &mut app.entry_form.as_mut().unwrap().list_state);
    }
